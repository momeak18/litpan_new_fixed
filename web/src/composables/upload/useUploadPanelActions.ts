...
export function useUploadPanelActions(ctx: UploadActionsCtx) {
  ...
  async function openUploadTaskPanel(preferredCategory: "" | "relay" | "offline" | "local-extract" = "") {
    store.uploadTaskPanelOpen.value = true;
    await Promise.all([
      stream.fetchUploadTasks(),
      deps.refreshOfflineTasks(true, true),
    ]);
    if (preferredCategory === "local-extract") {
      store.taskPanelCategory.value = "local-extract";
    } else if (preferredCategory === "offline") {
