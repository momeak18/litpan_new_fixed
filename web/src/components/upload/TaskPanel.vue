...
import { localExtractApi, type LocalExtractTask } from "@/api/localExtract";

type CategoryKey = "upload" | "relay" | "offline" | "local-extract";
...
const localExtractStateFilter = ref<StateKey>("active");
const panelRoot = ref<HTMLElement | null>(null);
const localExtractTasks = ref<LocalExtractTask[]>([]);
const localExtractLoading = ref(false);
const localExtractLoaded = ref(false);
let localExtractFetching = false;
let localExtractPollTimer: number | undefined;
...
async function fetchLocalExtractTasks(quiet = false) {
  if (localExtractFetching) return;
  localExtractFetching = true;
  if (!quiet && !localExtractLoaded.value) localExtractLoading.value = true;
  try {
    localExtractTasks.value = await localExtractApi.list();
    localExtractLoaded.value = true;
  } catch {
    // 认证失效等错误不覆盖当前任务快照，避免面板闪空。
  } finally {
    localExtractLoading.value = false;
    localExtractFetching = false;
  }
}
...
function localExtractStateOf(task: LocalExtractTask): StateKey {
  if (task.status === "success") return "done";
...
function buildLocalExtractRow(task: LocalExtractTask): PanelRow {
  const state = localExtractStateOf(task);
  const files = task.total_files > 0 ? `${task.uploaded_files}/${task.total_files} 个文件` : "等待解压";
  return {
    id: task.task_id,
    kind: "local-extract",
    raw: task,
    name: task.source_file_name,
    source: task.target_display_path || "/",
    status: task.error ? "失败" : task.message || task.status,
    statusDetail: task.error || task.message || "",
    statusClass: state === "done" ? "success" : state === "failed" ? "error" : "downloading",
    tail: state === "done" ? "已完成" : files,
    tailActive: state === "active",
    progress: Number(task.progress || 0),
    progressClass: state === "failed" ? "error" : "downloading",
    showProgress: state === "active",
    sortOrder: offlineTaskOrder(task),
    badgeLogo: "",
    badgeName: "解压",
    badgeColor: "#6a5af9",
    searchText: [task.source_file_name, task.target_display_path, task.message, task.error].join(" "),
  };
}
...
const localExtractRows = computed(() => localExtractTasks.value.map(buildLocalExtractRow));
...
function stateFilterOf(category: CategoryKey) {
  ...
  if (category === "local-extract") return localExtractStateFilter.value;
...
const baseRows = computed(() => {
  ...
  if (taskPanelCategory.value === "local-extract") return localExtractRows.value;
...
const currentRows = computed(() =>
  [...baseRows.value]
    .filter((row) => {
      ...
      if (taskPanelCategory.value === "local-extract" && localExtractStateOf(row.raw) !== filter) return false;
      return true;
    })
...
const canToggleSelected = computed(() => {
  ...
  return taskPanelCategory.value === "upload" || taskPanelCategory.value === "relay";
});
...
const tailColumnLabel = computed(() => {
  ...
  if (taskPanelCategory.value === "local-extract") return "进度";
  return "速度";
});
...
const emptyText = computed(() => {
  ...
  if (taskPanelCategory.value === "local-extract") {
    return localExtractStateFilter.value === "done" ? "暂无已完成本地解压任务" : localExtractStateFilter.value === "failed" ? "暂无失败本地解压任务" : "暂无进行中的本地解压任务";
  }
...
const showLoading = computed(() =>
  ...
  (taskPanelCategory.value === "local-extract" && localExtractLoading.value),
);
const loadingText = computed(() =>
  taskPanelCategory.value === "upload" ? uploadTaskPanelLoadingText?.value || "正在加载上传任务..." : taskPanelCategory.value === "local-extract" ? "正在加载本地解压任务..." : "正在加载离线任务...",
);

function countByState(category: CategoryKey, state: StateKey) {
  const rows = category === "upload" ? uploadRows.value : category === "relay" ? relayRows.value : category === "local-extract" ? localExtractRows.value : offlineRows.value;
  return rows.filter((row) => {
    if (category === "upload") return uploadStateOf(row.raw) === state;
    if (category === "relay") return relayStateOf(row.raw) === state;
    if (category === "local-extract") return localExtractStateOf(row.raw) === state;
...
const navCategories = computed(() => [
  {
    key: "local-extract" as const,
    label: "本地解压",
    icon: "package",
    count: countByState("local-extract", "active"),
    states: [
      { key: "active", label: "进行中", count: countByState("local-extract", "active"), active: localExtractStateFilter.value === "active", onClick: () => { localExtractStateFilter.value = "active"; } },
      { key: "done", label: "已完成", count: countByState("local-extract", "done"), active: localExtractStateFilter.value === "done", onClick: () => { localExtractStateFilter.value = "done"; } },
      { key: "failed", label: "失败", count: countByState("local-extract", "failed"), active: localExtractStateFilter.value === "failed", onClick: () => { localExtractStateFilter.value = "failed"; } },
    ],
  },
  {
    key: "upload" as const,
...
function showStatusPulse(row: PanelRow) {
  ...
  if (row.kind === "local-extract") {
    return ["queued", "downloading", "extracting", "uploading"].includes(String(row.raw?.status || ""));
  }
  return false;
}
...
async function handleSelectedDelete() {
  ...
  } else if (taskPanelCategory.value === "local-extract") {
    try {
      await Promise.all(rows.map((row) => localExtractApi.remove(row.id)));
      await fetchLocalExtractTasks(true);
      toast.success("本地解压任务已删除");
    } catch {
      toast.error("删除本地解压任务失败");
    }
  } else {
...
onMounted(() => {
  ...
  void fetchLocalExtractTasks();
  localExtractPollTimer = window.setInterval(() => {
    if (taskPanelCategory.value === "local-extract") void fetchLocalExtractTasks(true);
  }, 15000);
});
...
onUnmounted(() => {
  ...
  if (localExtractPollTimer !== undefined) window.clearInterval(localExtractPollTimer);
});
</script>
