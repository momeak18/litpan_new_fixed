...
const createItems = computed<DropdownMenuItem[]>(() => {
  ...
  if (props.localExtractSupported) {
    items.push({ key: "local-extract", label: "本地解压并上传", icon: "package", type: "action" });
  }
  return items;
});
...
function onCreateSelect(key: string) {
  ...
  else if (key === "offline-download") emit("offline-download");
  else emit("local-extract");
}
...
<template>
  <div class="file-toolbar" :class="{ 'file-toolbar--compact': compactHome }">
    ...
    <div class="file-toolbar__left">
      ...
      <AppButton
        v-if="isAdmin && (archiveCount || 0) > 0"
        variant="secondary"
        class="file-toolbar__btn"
        :title="`选中当前目录的 ${archiveCount} 个压缩包`"
        @click="emit('select-archives')"
      >
        <span class="file-toolbar__icon"><SvgIcon name="package" :size="17" /></span>
        <span>选压缩包 ({{ archiveCount }})</span>
      </AppButton>
