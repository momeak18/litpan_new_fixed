...
import { localExtractApi } from "@/api/localExtract";
...
const supportedArchiveNames = new Set(["zip", "rar", "7z"]);
const currentArchives = computed(() => files.value.filter((item) => !item.is_dir && supportedArchiveNames.has(item.name.split(".").pop()?.toLowerCase() || "")));
const selectedArchives = computed(() => selectedFiles.value.filter((item) => !item.is_dir && supportedArchiveNames.has(item.name.split(".").pop()?.toLowerCase() || "")));

function selectCurrentArchives() {
  selectedIds.value = currentArchives.value.map(fileKey);
  if (selectedIds.value.length > 0) {
    toast.success(`已选中 ${selectedIds.value.length} 个压缩包`);
  }
}

async function createLocalExtractTasks() {
  if (!currentAccountId.value || selectedArchives.value.length === 0) {
    toast.warning("请先选择一个或多个 .zip、.rar、.7z 压缩包");
    return;
  }
  const result = await showConfirm({
    title: "本地解压并上传",
    message: `将串行处理 ${selectedArchives.value.length} 个压缩包：下载到本地、7-Zip 解压、上传回当前目录。成功后会删除临时文件。`,
    confirmText: "加入队列",
    danger: false,
  });
  if (result.action !== "confirm") return;
  try {
    await localExtractApi.create({
      account_id: currentAccountId.value,
      source_file_ids: selectedArchives.value.map((item) => item.id),
      source_file_names: selectedArchives.value.map((item) => item.name),
      target_parent_id: currentParentId.value,
      target_display_path: getCurrentDisplayPath(),
    });
    selectedIds.value = [];
    await uploadApi.openUploadTaskPanel("local-extract");
    toast.success("已加入本地解压队列（默认单任务串行）");
  } catch (err) {
    toast.error(getApiErrorMessage(err, "创建本地解压任务失败"));
  }
}
  ...
  <div class="browser" :class="{ 'browser--floating-accounts': floatingAccountSwitchEnabled }">
    ...
    <div v-else-if="showBrowserFrame" class="browser__frame" :class="{ 'browser__frame--grid': view === 'grid' }">
      ...
      <FileToolbar
        ...
        :local-extract-supported="isAdmin"
        :archive-count="currentArchives.length"
        ...
        @local-extract="createLocalExtractTasks"
        @select-archives="selectCurrentArchives"
